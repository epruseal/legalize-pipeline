...
def test_original_error_and_cache_survive_failure(monkeypatch, caplog, cached):
    ...
    monkeypatch.setattr(api, 'parse_xml', capture)
...
def test_existing_entity_only_cache_is_updated(monkeypatch):
    raw = document('<조문내용>A & B</조문내용>')
    cache.put_detail('123', raw)
    monkeypatch.setattr(api, '_request', lambda *a, **kw: pytest.fail('Unexpected network'))
    result = api.get_law_detail('123')
    assert result['articles'][0]['조문내용'] == 'A & B'
    assert cache.get_detail('123') == result['raw_xml']
    ET.fromstring(result['raw_xml'])
