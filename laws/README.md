...
`(공포일자, 공포번호, MST)` 순이다 — 같은 날 공포된 다른 MST도 정확히 구분된다.

`converter.entry_sort_key`는 여기에 법령명이 더 들어간 `(공포일자, 법령명,
공포번호, MST)`이지만, 비교 대상이 **한 파일 안의 버전들**이라 법령명이 상수여서
결과 순서는 같다. 실제로 캐시 84,556건을 훑어도 같은 법령ID·같은 공포일자에
법령명이 다른 사례는 0건이다.

HEAD 값은 커밋된 마크다운의 frontmatter에서 읽는데, 작성 경로에 따라
`공포일자: 2024-01-15`(API 경로)와 `공포일자: '2024-01-15'`(CSV 경로) 두 형식이
모두 나온다. 따옴표를 벗기지 않으면 `'`가 모든 숫자보다 작아 기준선이 항상
최하위로 밀리고 가드가 조용히 무력화되므로, `_frontmatter_value()`가 두 형식을
모두 처리한다.
